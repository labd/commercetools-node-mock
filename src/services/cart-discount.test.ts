...
describe('Cart Discounts Query', () => {
    ...
    test('no filter', async () => {
        ...
        expect(myRelativeCartDiscount.description).toStrictEqual({ en: 'My relative cart discount' })
        expect(myRelativeCartDiscount.isActive).toBe(false)
        expect(myRelativeCartDiscount.name).toStrictEqual({ en: 'myRelativeCartDiscount' })
        expect(myRelativeCartDiscount.stores).toStrictEqual([])
        expect(myRelativeCartDiscount.references).toStrictEqual([])
        expect(myRelativeCartDiscount.target).toStrictEqual({ type: 'lineItems', predicate: '1=1' })
        expect(myRelativeCartDiscount.requiresDiscountCode).toBe(false)
        expect(myRelativeCartDiscount.stackingMode).toBe('Stacking')
        expect(myRelativeCartDiscount.value).toStrictEqual({ type: 'relative', permyriad: 1000 })
...
describe('Cart Discounts Update Actions', () => {
    ...
    const createCartDiscount = async (typeId) => {
        ...
        const response = await supertest(ctMock.app).post('/dummy/cart-discounts').send(cartDiscountDraft)
        expect(response.status).toBe(201)
    ...
    test('set description', async () => {
        ...
        const response = await supertest(ctMock.app)
            ...
            .send({
                version: 1,
                actions: [{ action: 'setDescription', description: { en: 'Description' } }],
            })
        expect(response.status).toBe(200)
        expect(response.body.version).toBe(2)
    ...
    test('set valid from', async () => {
        ...
        const response = await supertest(ctMock.app)
            ...
            .send({
                version: 1,
                actions: [{ action: 'setValidFrom', validFrom: '2020-01-01T00:00:01.000Z' }],
            })
        expect(response.status).toBe(200)
        expect(response.body.version).toBe(2)
    ...
    test('set valid until', async () => {
        ...
        const response = await supertest(ctMock.app)
            ...
            .send({
                version: 1,
                actions: [{ action: 'setValidUntil', validUntil: '2020-12-31T23:59:59.999Z' }],
            })
        expect(response.status).toBe(200)
        expect(response.body.version).toBe(2)
    ...
    test('change target', async () => {
        ...
        const response = await supertest(ctMock.app)
            ...
            .send({
                version: 1,
                actions: [{ action: 'changeTarget', target: { type: 'shippingInfo', predicate: '2=2' } }],
            })
        expect(response.status).toBe(200)
        expect(response.body.version).toBe(2)
        expect(response.body.target).toStrictEqual({ type: 'shippingInfo', predicate: '2=2' })
    })
